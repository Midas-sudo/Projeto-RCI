...
int get_command(char **args)
{
    ...
    memset(args[4], '\0', BUFFER_SIZE);
    memset(args[5], '\0', BUFFER_SIZE);
...
int setup(fd_set *available_sockets, Node_struct *node)
{
    int udp_fd, tcp_fd, max_socket;
    ...
    node->is_online = 1;
    max_socket = udp_fd > tcp_fd ? udp_fd + 1 : tcp_fd + 1;
...
int main(int argc, char **argv)
{
    ...
    int UDP_socket, TCP_socket, TCP_Prev_socket, max_socket, error, n, new_fd, dist, find_id;
    char message[BUFFER_SIZE];
    FILE *fp;
    Node_struct *node;
        ...
        for (int i = 0; i < max_socket; i++)
        {
            if (FD_ISSET(i, &ready_sockets))
            {
                if (i == STDIN_FILENO) // General Listen STDIN
                {
                    ...
                    {
                    case 0: // new
                        max_socket = setup(&available_sockets, node);
                        break;
                ...
                else if (i == UDP_socket) // General Listen UDP
                {
                    memset(message, '\0', BUFFER_SIZE);
